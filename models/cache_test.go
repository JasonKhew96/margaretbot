...
func testCachesExists(t *testing.T) {
	...
	e, err := CacheExists(ctx, tx, o.VideoID)
	if err != nil {
...
func testCachesFind(t *testing.T) {
	...
	cacheFound, err := FindCache(ctx, tx, o.VideoID)
	if err != nil {
		t.Error(err)
	}
...
func testCachesInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(cachePrimaryKeyColumns, cacheColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
var (
	cacheDBTypes = map[string]string{`VideoID`: `TEXT`, `CreatedAt`: `TIMESTAMP`, `UpdatedAt`: `TIMESTAMP`, `IsPublished`: `BOOLEAN`, `IsScheduled`: `BOOLEAN`}
	_            = bytes.MinRead
)
