...
func FindCache(ctx context.Context, exec boil.ContextExecutor, videoID string, selectCols ...string) (*Cache, error) {
	cacheObj := &Cache{}
	...
	query := fmt.Sprintf(
		"select %s from \"cache\" where \"video_id\"=?", sel,
	)

	q := queries.Raw(query, videoID)
...
func (o *Cache) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQuerySQLite(dialect, "\"cache\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Cache) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"cache\" WHERE \"video_id\"=?"
...
func (o *Cache) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindCache(ctx, exec, o.VideoID)
	if err != nil {
		return err
	}
...
func CacheExists(ctx context.Context, exec boil.ContextExecutor, videoID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"cache\" where \"video_id\"=? limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, videoID)
	}
	row := exec.QueryRowContext(ctx, sql, videoID)
...
func (o *Cache) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return CacheExists(ctx, exec, o.VideoID)
}
